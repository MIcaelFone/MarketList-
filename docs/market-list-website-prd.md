...
- Public landing and marketing pages must include unique `title` and `meta description` tags.
- Public pages must render semantic heading structure (`h1` + ordered `h2/h3`) for crawlability.
- Sitemap and robots directives must be present before production release.
- Lighthouse SEO score target for public pages: >= 90.

## 8. Accessibility Level

- Target standard: WCAG 2.1 AA for all MVP user journeys.
- Core flows must be fully keyboard navigable.
- Color must not be the only status indicator (status labels accompany color states).
- Accessibility checks run before release using automated tooling plus manual keyboard testing.

## 9. Data Model (Local Persistence)

### 9.1 Storage Key
- `market_list_app_v1`

### 9.2 Suggested JSON Structure
```json
{
  "budget": {
    "amount": 1200,
...
## 13. Technical Notes for Development

- Suggested implementation stack:
  - Next.js (App Router)
  - Tailwind CSS
  - TypeScript
- Use React state with synchronization to local browser storage.
- Create utility functions:
  - `calculateItemSubtotal(quantity, unitPrice)`
  - `calculateListTotal(items)`
  - `calculateBudgetStatus(total, budgetAmount)`
- Centralize currency formatting with:
  - `Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" })`

## 14. Definition of Done

- All MVP functional requirements delivered
- Responsive layout validated on mobile and desktop
- Persistence working for budget and lists
- Basic validation implemented (no negative quantity/price)
